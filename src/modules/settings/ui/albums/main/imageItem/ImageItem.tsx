...
export function ImageItem(props: ImageItemProps) {
	const { uri, isLoading, id, isVisible, onDelete, onChangeVisibility } = props;
	return (
		<View style={styles.container}>
			...
			{id && !isLoading && onDelete && onChangeVisibility && (
				<View style={styles.actions}>
					<Button
						variant="outline"
						onPress={() => onChangeVisibility(id)}
						iconLeft={isVisible ? <Icons.EyeClosedIcon color={COLORS.plum} /> : <Icons.EyeOpenedIcon color={COLORS.plum} />}
					/>
					<Button
						variant="outline"
						onPress={() => onDelete(id)}
